...
const UI = {
    ...
    addChatLog: function (s, t) {
        ...
        const ttsBtn = isAI ? `<button class="tts-btn" title="Vorlesen" onclick="TTS.speak(this.closest('.tts-msg').querySelector('.tts-text'), this)"><i class="fas fa-volume-up"></i></button>` : '';
        d.innerHTML = `<div class="${titleSizeClass} ${titleColorClass}">${s}${ttsBtn}</div><div class="tts-text ${textSizeClass}">${formattedText}</div>`;
    ...
    showDetails: function (id) {
        ...
        const xpNeeded = Math.floor(100 * Math.pow(c.level, 1.4));
