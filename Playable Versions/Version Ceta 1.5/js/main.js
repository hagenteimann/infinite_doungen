...
function showFloatingNumber(targetName, amount, type) {
    ...
    // Use fixed positioning to avoid clipping in overflow containers
...
function showLevelUpAnimation(char) {
    ...
    // Layer 1: Apply glow effect to card
    targetCard.classList.add('level-up-glow');
    setTimeout(() => {
        targetCard.classList.remove('level-up-glow');
    }, 2000);

    // Layer 2: Floating "LEVEL UP!" text (positioned to avoid clipping)
    const float = document.createElement('div');
    ...
    // Layer 3: Animate the level number specifically
    setTimeout(() => {
...
const PartyManager = {
    ...
    getEffectiveMaxHp: function (char) {
        ...
        const conBonus = (effAttrs.CON - 10) * 3;
    ...
    addXP: function (char, amount) {
        ...
        let needed = Math.floor(100 * Math.pow(char.level, 1.4)); let leveledUp = false; let levelUpsThisChar = 0;
        while (char.xp >= needed) {
            char.xp -= needed; char.level++; char.statPoints += 2;
            // Klassen-Bonus: Automatischer Stat-Punkt basierend auf Klasse
            const classBonusMap = {
            ...
            const bonusStat = classBonusMap[char.class];
            if (bonusStat && char.attributes[bonusStat] !== undefined) {
                char.attributes[bonusStat]++;
            }
            char.hp = this.getEffectiveMaxHp(char);
            leveledUp = true; levelUpsThisChar++; needed = Math.floor(100 * Math.pow(char.level, 1.4));
...
const CombatManager = {
    ...
    endCombat: function () {
        ...
        // Auto-XP: verteile XP basierend auf bezwungenen Gegnern + Dungeon-Tiefe
        const dungeonBonus = (State.dungeonLevel || 0) * 8;
        const xpPool = State.defeatedEnemies.reduce((sum, e) => {
            if (e.fled) return sum; // Geflohene Feinde geben keine XP
            return sum + Math.floor((e.maxHp || 20) * 1.2) + 10;
        }, 0) + dungeonBonus;
        const livingHeroes = State.party.filter(p => !p.isSummon && p.hp > 0);
        if (xpPool > 0 && livingHeroes.length > 0) {
            const xpEach = Math.max(15, Math.floor(xpPool / livingHeroes.length));
        ...
        // Auto-Gold: Goldmünzen basierend auf Gegnerstärke
        const goldPool = State.defeatedEnemies.reduce((sum, e) => {
            if (e.fled) return sum + Math.floor((e.maxHp || 20) * 0.1); // Geflohene lassen wenig fallen
            return sum + Math.floor((e.maxHp || 20) * 0.5) + 5;
        }, 0);
        if (goldPool > 0) {
            State.gold = (State.gold || 0) + goldPool;
            Sound.play('loot');
...
const TagParser = {
    ...
    applyEvent: function (type, targetName, amount, rawArgs) {
        ...
        if (type === 'gegnerschaden') {
            ...
            // [DeathSave: Name] – Automatischer Todesrettungswurf
...
const Engine = {
    ...
    interactWithAI: async function (actionMsg) {
        ...
        // Undo-Snapshot vor jeder Aktion sichern
        try {
    ...
    camp: function () {
        ...
        const reduction = hasProvisions ? 14 : 8;
    ...
    submitPendingRolls: function () {
        const rollsCopy = [...State.pendingRolls]; // Kopie für Momentum-Auswertung
        ...
        // Momentum-Tracking: Alle Erfolge = +1, sonst Reset
    ...
    generatePlotTwist: async function () {
        ...
        try {
            ...
            // Verarbeite wie eine normale DM-Antwort
    ...
    useAbility: function (cid, abilityName, isItemAbility = false) {
        ...
        if (c && ab) {
            ...
            // Auto-set cooldown for summon abilities (8 rounds)
            if (isSummonAbility) {
                let cdKey2 = c.id + '_' + ab;
                State.abilityCooldowns[cdKey2] = 8;
            }
            DOM.playerInput.focus();
            UI.hideDetails();
        }
    ...
    undoLastAction: function () {
        ...
        State.party = snap.party.map(c => Utils.sanitizeCharacter(c));
        State.activeEnemies = snap.activeEnemies || [];
        State.defeatedEnemies = snap.defeatedEnemies || [];
        State.lootDrops = snap.lootDrops || [];
        State.fate = snap.fate || 0;
        State.fatigue = snap.fatigue || 0;
        State.gold = snap.gold || 0;
        State.activeMerchant = snap.activeMerchant || null;
        State.abilityCooldowns = snap.abilityCooldowns || {};
